...
from gbcommon.types.gbenvconfig import (
    ...
    resolve_registered_environment,
)
...
def gb_env_formating(value: str, type: str) -> str:
    ...
    resolves to its canonical registry key: ``gb_env_normalize`` only knows the
    built-in names, so without this check a valid custom environment would reach
    ``sys.exit`` and terminate the process — fatal for a long-running server that
    embeds gbcli. Genuinely invalid names still exit, preserving CLI behavior.
    """
    load_extra_environment_configs()
    registered = resolve_registered_environment(value)
