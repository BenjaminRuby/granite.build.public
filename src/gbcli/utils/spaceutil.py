...
def has_local_profile_store() -> bool:
    """Return True if this environment keeps spaces/profiles in the local config.

    An environment with empty ``config_spaces``/``config_profile`` has no section to
    read or write that state in, so spaces must be resolved live from gbserver. That
    covers STANDALONE and any environment configured to point at a separate gbserver
    deployment; the built-in PROD/STAGING/DEV keep non-empty section names and so keep
    using the local profile store.
